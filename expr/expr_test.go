...
func TestEvalExpression(t *testing.T) {
	...
	tests := []th.EvalTestItem{
		...
		{
			parser.NewExpr("round",
				"metric1", 3,
			),
			map[parser.MetricRequest][]*types.MetricData{
				{"metric1", 0, 1}: {types.MakeMetricData("metric1", []float64{0.5, 2.298, math.NaN(), 91.019, -524.82, 245}, 1, now32)},
			},
			[]*types.MetricData{types.MakeMetricData("round(metric1,3)", []float64{0.5, 2.298, math.NaN(), 91.019, -524.82, 245}, 1, now32)},
			nil,
		},
		{
			parser.NewExpr("round",
			...
			[]*types.MetricData{types.MakeMetricData("round(metric1,1)", []float64{0.5, 2.3, math.NaN(), 91.0, -524.8, 245}, 1, now32)},
			nil,
		},
		{
			parser.NewExpr("round",
			...
			[]*types.MetricData{types.MakeMetricData("round(metric1)", []float64{0, 2, 2, math.NaN(), 91, -525, 245}, 1, now32)},
			nil,
		},
		{
			parser.NewExpr("round",
			...
			[]*types.MetricData{types.MakeMetricData("round(metric1,-2)", []float64{0, 0, math.NaN(), 100, -500, 300}, 1, now32)},
			nil,
		},
		{
			parser.NewExpr("round",
			...
			[]*types.MetricData{types.MakeMetricData("round(metric1,-10)", []float64{0, 0, math.NaN(), 0, 0, 0}, 1, now32)},
			nil,
		},
		{
		...
		},
		{
			...
			[]*types.MetricData{types.MakeMetricData("metric1.hello",
				[]float64{1, 2, 3, 4, 5}, 1, now32)},
			nil,
		},
		{
